...
import {
  Profile,
  ProfileCompleteOnboardingResponse,
  ProfileMergeResumeResponse,
  ProfileRetrieveResponse,
  ProfileSaveOnboardingStepParams,
  ProfileSaveOnboardingStepResponse,
  ProfileUpdatePreferencesParams,
  ProfileUpdatePreferencesResponse,
} from './resources/profile';
import {
  RecommendationListResponse,
  RecommendationRefreshBatchParams,
...
} from './resources/resume';
import {
  RoadmapGenerateParams,
  RoadmapGenerateResponse,
  RoadmapListResponse,
  Roadmaps,
} from './resources/roadmaps';
import {
  Taxonomy,
  TaxonomyListFunctionsResponse,
  TaxonomyListRolesParams,
  TaxonomyListRolesResponse,
  TaxonomySearchSkillsParams,
  TaxonomySearchSkillsResponse,
} from './resources/taxonomy';
import {
  AlignmentGetResourcesStatusResponse,
  AlignmentListResponse,
  AlignmentRetrieveResponse,
  Alignments,
} from './resources/alignments/alignments';
import {
  JobsBoard,
  JobsBoardRefreshParams,
  JobsBoardRefreshResponse,
} from './resources/jobs-board/jobs-board';
import {
  LearningPathEnrollParams,
  LearningPathEnrollResponse,
  LearningPathListResponse,
  LearningPathRetrieveResponse,
  LearningPaths,
} from './resources/learning-paths/learning-paths';
import {
  NotificationDeliverParams,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['RESALIGN_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['RESALIGN_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['RESALIGN_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Resalign API.
 */
export class Resalign {
  apiKey: string | null;
   ...
   * API Client for interfacing with the Resalign API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['RESALIGN_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['RESALIGN_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('RESALIGN_BASE_URL'),
    apiKey = readEnv('RESALIGN_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Resalign.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('RESALIGN_LOG'), "process.env['RESALIGN_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('RESALIGN_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static Resalign = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static ResalignError = Errors.ResalignError;
  ...
  alignments: API.Alignments = new API.Alignments(this);
  analysis: API.Analysis = new API.Analysis(this);
  roadmaps: API.Roadmaps = new API.Roadmaps(this);
  learningPaths: API.LearningPaths = new API.LearningPaths(this);
  companyResearch: API.CompanyResearch = new API.CompanyResearch(this);
  jobsBoard: API.JobsBoard = new API.JobsBoard(this);
  favorites: API.Favorites = new API.Favorites(this);
  resume: API.Resume = new API.Resume(this);
  profile: API.Profile = new API.Profile(this);
  recommendations: API.Recommendations = new API.Recommendations(this);
  assessments: API.Assessments = new API.Assessments(this);
  notifications: API.Notifications = new API.Notifications(this);
}

Resalign.Health = Health;
Resalign.Taxonomy = Taxonomy;
Resalign.Alignments = Alignments;
Resalign.Analysis = Analysis;
Resalign.Roadmaps = Roadmaps;
Resalign.LearningPaths = LearningPaths;
Resalign.CompanyResearch = CompanyResearch;
Resalign.JobsBoard = JobsBoard;
Resalign.Favorites = Favorites;
Resalign.Resume = Resume;
Resalign.Profile = Profile;
Resalign.Recommendations = Recommendations;
Resalign.Assessments = Assessments;
Resalign.Notifications = Notifications;

export declare namespace Resalign {
  ...
  export {
    LearningPaths as LearningPaths,
